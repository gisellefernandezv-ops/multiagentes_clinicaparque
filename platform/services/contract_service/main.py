...
def ingest_contract(filename: str, text: str) -> dict:
    ...
    # Eliminar colección previa (idempotente)
    try:
...
def check_contract(supplier_id: str, amount: float = Query(...)):
    ...
    try:
        ...
        query = f"contrato proveedor {sid} monto máximo autorizado por factura límite"
        results = collection.query(query_texts=[query], n_results=2)

        documents = results.get("documents", [[]])[0]
        metadatas = results.get("metadatas", [[]])[0]

        # Filtrar por supplier_id
        chosen = [
            d for d, m in zip(documents, metadatas)
            if m.get("supplier_id", "").upper() == sid
        ] or documents
